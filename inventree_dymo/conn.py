...
class Conn:
    ...
    def send_label(self, index: int, png: Image) -> None:
        """
        :param index: A counter to use with status reporting.
        :param png: The label image as intended to be read (i.e. landscape
        aspect ratio). It is imperative that the image has been rasterized at
        Conn.DPI resolution.
        """
        label_width_px, label_length_px = png.size

        # Convert to 1 BPP using a hard threshold.
        data = png.rotate(90, expand=1).convert('L').point(lambda x: 0 if x > 200 else 1, mode='1').tobytes()
        ...
        self.send_command(
            ...
            label_length_px & 0xFF,
            (label_length_px >> 8) & 0xFF,
            (label_length_px >> 16) & 0xFF,
            (label_length_px >> 24) & 0xFF,

            label_width_px & 0xFF,
            (label_width_px >> 8) & 0xFF,
            (label_width_px >> 16) & 0xFF,
            (label_width_px >> 24) & 0xFF
        )
    ...
    def wait_for_job_completion(self, label_count: int) -> StatusReport:
        """
        wait_for_job_completion polls the printer until the label index status
        register has reached the expected count and the print head is idle.

        :param label_count: The expected number of labels within the print job.
        """
        while True:
            report = self.status_report(intent=LockIntent.NONE)
            logger.debug(f"machine is printing label {report.label_index} of {label_count}")
            match report.printer_state:
                ...
                case PrinterState.ERROR:
                    raise StatusReportError("machine reports error status", report)

                case _:
                    raise StatusReportError("unexpected machine status", report)

    def wait_until_state(self,
                         until: PrinterState,
                         deadline: float = time.time() + DEFAULT_TIMEOUT,
                         intent: LockIntent = LockIntent.NONE,
                         ) -> StatusReport:
        """
        wait_until_state loops until the printer is in the required state
        before returning. A StatusReportError will be raised if the printer has
        not reached the desired state before the deadline.
        """
        while True:
            ...
            if time.time() >= deadline:
                raise StatusReportError("deadline exceeded", report)
